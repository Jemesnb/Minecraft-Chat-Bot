...
- **Web 管理界面**  
  内置网页配置工具，通过浏览器即可编辑 `.env` 文件，支持可选的 Basic 认证，方便远程管理。
  默认用户为admin，
  密码为12345，
  访问地址为http://<你的服务器公网IP>:3000，
  默认是3000，可以在配置文件自定义。

- **CDK系统**
  可在游戏内使用#bot help获得使用方法
...
所有配置均通过环境变量进行。需要复制 `.env.example` 文件，并移除后面的`.example`放在项目根目录，启动器会自动加载。
